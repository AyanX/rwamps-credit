...
const ProductsPage = () => {
  ...
  const [form, setForm] = useState<Omit<Product, 'id'>>({ icon: '', title: '', content: '', points: [''], bg_color: '#22C55E' });
  ...
  const openEdit = (product: Product) => {
    setEditItem(product);
    setForm({ icon: product.icon, title: product.title, content: product.content, points: [...product.points], bg_color: product.bg_color });
    setModal(true);
  };

  const openAdd = () => {
    setEditItem(null);
    setForm({ icon: '', title: '', content: '', points: [''], bg_color: '#22C55E' });
    setModal(true);
  };
